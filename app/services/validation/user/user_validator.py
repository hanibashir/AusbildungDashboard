...
class UserValidator:
    ...
    def __init__(self, data=None, profile=False):
    ...
    def validate_registration_input(self) -> tuple[bool, str]:
    ...
    def validate_profile_update_input(self) -> tuple[bool, str]:
        if self.__name:
            if not self.validate_name():
                return False, message('user', 'NAME_NOT_VALID')
        else:
        ...
        # check password confirm not empty
        if self.__password and not self.__confirm_password:
            return False, message('user', 'EMPTY_FIELD', 'c_password')
